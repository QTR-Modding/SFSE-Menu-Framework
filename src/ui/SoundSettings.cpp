...
namespace SFSEMenuFramework::SoundSettings
{
    ...
    void Render(bool& saveFailed)
    {
        ...
        bool changed = ImGui::Checkbox("Enable menu sounds", &sounds.Enabled);
        int volume = static_cast<int>(sounds.Volume * 100 + 0.5F);
        if (ImGui::SliderInt("Sound volume", &volume, 0, 100, "%d%%", ImGuiSliderFlags_AlwaysClamp)) {
            sounds.Volume = static_cast<float>(volume) / 100;
            changed = true;
        }
        ImGui::TextWrapped("Choose a sound for each event. Default uses a soft built-in tone.");
        ImGui::TextWrapped("Custom files: Data/SFSE/Plugins/SFSEMenuFrameworkSounds. PCM16 WAV, mono/stereo, 8-192 kHz, up to 5 seconds and 4 MiB.");
        if (ImGui::Button("Reload sounds")) { Discover(); Audio::Reload(); }
        for (std::size_t i = 0; i < Audio::eventCount; ++i) {
            auto& binding = sounds.Bindings[i];
            ImGui::PushID(static_cast<int>(i));
            changed = ImGui::Checkbox(Audio::descriptions[i].Label, &binding.Enabled) || changed;
            ImGui::SetNextItemWidth(ImGui::GetContentRegionAvail().x * 0.7F);
            const char* preview = binding.File == Audio::defaultFile ? "Default" : binding.File.data();
            if (ImGui::BeginCombo("##File", preview)) {
                if (ImGui::Selectable("Default", binding.File == Audio::defaultFile)) {
        ...
        if (saveFailed && ImGui::Button("Retry saving sounds")) saveFailed = !FrameworkSettings::Save();
